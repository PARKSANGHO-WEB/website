...
$attach_count_1 = trim(sqlfilter($_REQUEST['attach_count_1'])); 
	...
	$sql_pre1 = "select idx from member_register_survey where 1 and member_idx='".$member_idx."'"; 
	$result_pre1  = mysqli_query($gconnet,$sql_pre1);
	if(mysqli_num_rows($result_pre1) > 0) {
		$query = " update member_register_survey set "; 
		...
		$query .= " mdate = now()";
		$query .= " where member_idx='".$member_idx."'";
		//echo $query; exit;
		...
		if(mysqli_num_rows($mem_info_query) == 0){
			error_frame("There are no registered members.");
		}
