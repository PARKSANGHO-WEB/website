		...
		if($pwd == $actualPwd['pwd']) {
			if($funct == "1") {
				...
				if($result['b_file1'] === null) {
					...
					echo "<script>alert('게시글이 삭제되었습니다.'); window.location.href ='review.php'; </script>";
					exit();
				} else {
					$fileDestination = "../../upload_file/rv/" . $result['b_file1'];
					unlink($fileDestination);
					$sql = 'DELETE FROM ' .$tableName. ' WHERE idx ="' .$idx. '"';
					$removeIt = mysqli_query($gconnet, $sql);
					echo "<script>alert('이미지 게시글이 삭제되었습니다.'); window.location.href ='review.php'; </script>";
					exit();
				}
			} else {
...
<body>
    ...
    <div id="container">
        ...
        <div id="newContent">
            <form name="frm" action="review_modi_back.php" method="POST" enctype="multipart/form-data">
                <table>
                    <tr>
                        <th><label for="text_name">성명</label></th>
                        <td><input type="text" class="content" id="text_name" name="name" value="<?=$contents['name']?>" HNAME="성명" REQUIRED MAXBYTE="50" readonly ></td>
                    </tr>
                    <tr>
                        <th><label for="text_email">이메일</label></th>
                        <td><input type="email" class="content" id="text_email" name="email" value="<?=$contents['email']?>" HNAME="이메일" REQUIRED MAXBYTE="50" readonly ></td>
                    </tr>
                    <tr>
