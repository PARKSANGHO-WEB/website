...
<?
    ...
    //regflag 당첨여부[5:당첨 6:예비당첨 7: 당첨후취소  8:취소 9:탈락] 
    //regflag_two 당첨취소 상태 2:당첨취소신청, 3.당첨취소확정, 4.당첨취소불가

    $sql = " SELECT a.ridx, a.chasu,  b.comname, date_format(a.cymd, '%Y-%m-%d') as cymd, a.useday, a.regflag, a.regflag_two, date_format(a.cancel_date, '%Y.%m.%d') as cancel_date ";
    $sql .= "   FROM tb_reInfo a, tb_hu b ";
    $sql .= "   WHERE a.hidx = b.idx      ";
    $where = " AND a.cidx = '{$_COMPANY_ID}'   AND a.sano = '{$_SESSION['EMP_NO']}'  ";
    $where .= " AND (a.regflag in ( '7','8' ) OR a.regflag_two in ('2','3','4') ) ";
...
<input type="hidden" name="ridx" id="ridx">
    ...
    <div id="container">
        ...
        <div id="table">
            ...
            <table id="reserv-t" class="reserv-can">
                <tr>
                    <th>번호</th>
                    <th>지망</th>
                    <th>휴양소명</th> 
