...
if($bbs_code == "reviews"){
	$query = "select a.*,b.user_id,b.user_name,b.file_chg from tb_rv a inner join member_info b on a.product_idx=b.idx where 1=1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;
	$query_cnt = "select a.idx from tb_rv a inner join member_info b on a.product_idx=b.idx where 1=1 ".$where;
} else {
	$query = "select * from tb_rv a where 1=1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;
		...
		<div class="center-con">
			...
			<form name="s_mem" id="s_mem" method="post" action="<?php $_SERVER[PHP_SELP] ?>">
					...
					<table id="review">
                        ...
                        ?>
							<tr onclick="location.href='./review-view.php?idx=<?=$row[idx]?>'">
								<td onclick="event.cancelBubble=true"><input type="checkbox"  id="review[]" name="review[]" required="yes"  message="체크" value="<?=$row[idx]?>"></td>
								<td><?=$listnum?></td>
								<td><?=$row[title]?></td>
								<td><?=$row[name]?></td>
