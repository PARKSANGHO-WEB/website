...
if($bbs_code == "reviews"){
	$query = "select a.*,b.user_id,b.user_name,b.file_chg from tb_qa a inner join member_info b on a.product_idx=b.idx where 1=1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;
	$query_cnt = "select a.idx from tb_qa a inner join member_info b on a.product_idx=b.idx where 1=1 ".$where;
} else {
	$query = "select * from tb_qa a where 1=1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;
	$query_cnt = "select idx from tb_qa a where 1=1 ".$where;
}
...
<!--
    ...
    function go_tot_del() {
        ...
        if(check) {
            if(confirm('선택하신 질문을 삭제 하시겠습니까?')){
                frm.action = "qna_list_del_action.php";
                frm.submit();
            }
        } else {
            false;
...
<body>
    ...
    ?>
	<div class="contents-qna contents">
    <? 
        $MENU_DEPTH1 = "1";
        $MENU_DEPTH2 = "2";
    ...
    ?>
		<div class="center-con">
			...
			<form name="s_mem" id="s_mem" method="post" action="<?php $_SERVER[PHP_SELP] ?>">
					...
					<div class="all-c">
						<input type="checkbox" id="hostPall" data-check-pattern="[name^='qna']" name="hostPall">
					...
					<table id="qna">
                        ...
                        for ($i=0; $i<mysqli_num_rows($result); $i++){
                            ...
                            $reg_time3 = to_time(substr($row[nows],0,10));
                        ...
                        ?>
							<tr onclick="location.href='./qna-reple.php?idx=<?=$row[idx]?>'">
								<td onclick="event.cancelBubble=true"><input type="checkbox"  id="qna[]" name="qna[]" required="yes"  message="체크" value="<?=$row[idx]?>"></td>
                                <td><?=$listnum?></td>
								<td><?=$row[title]?></td>
								<td><?=$row[name]?></td>
								<td><?=substr($row[nows],0,11)?></td>
								<td><?=$row[hit]?></td>
								<td onclick="event.cancelBubble=true"><button type="button" onclick="location.href='./qna-reple.php?idx=<?=$row[idx]?>'">답변등록</button></td>
							</tr>
						<?}?>
