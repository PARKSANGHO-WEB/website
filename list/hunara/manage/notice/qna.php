...
function getList(){
    $("#list").jqGrid({
		...
		datatype: "json",
		mtype: "post",
		colNames: [ "선택", "번호", "제목", "작성자", "작성일", "조회수","답변등록"],
		colModel: [
...
function checkBox(cellvalue, options, rowObject){
        var idx = cellvalue;
        var str = "<input type='checkbox' id='qna[]' name='qna[]' required='yes'  message='체크' value='"+idx+"'";
...
var val1 = rowObject[0];
	...
	str += "<button id='modi' type=\"button\" onclick=\"location.href='./qna-reple.php?idx="+val1+"'\">답변등록</button>";   
...
function view(idx){
    ...
    document.location.href = "qna-reple.php?idx="+idx;
}
...
<!--
    ...
    function go_tot_del() {
        ...
        /*const query = 'input[name="qna[]"]:checked';
        const selectedEls = 
        ...
        // 출력
        if(result){
            location.href='./qna_list_del_action.php?idx='+result.slice(0,-1)+'';
