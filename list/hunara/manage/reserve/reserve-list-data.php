...
if(!empty($endDate)){
    ...
    if($searchDate == "cymd"){
        ...
        $WHERE .= " AND a.cancel_date <= date_format('$endDate', '%Y-%m-%d' )  ";
    }
}
...
$SQL .= "   a.name, a.sano, ifnull(a.hp, a.tel) as hp, a.regflag, a.regflag_two, a.midx, a.hidx, a.cidx, a.seq,  ";
$SQL .= "   date_format(a.cancel_date,'%Y.%m.%d') as cancel_date, case when a.chasu = 0 then '선착순' else concat(a.chasu,'지망') end as chasu ";
...
for ($i=0; $i<$totalCnt; $i++){
	...
	$responce->rows[$i]['cell']= array( $row['ridx'], ($totalCnt-$i), $row['name'], $row['chasu'], $row['wdate'], $useday, $row['cname'], 
                                        $row['pgubun'], $row['hname'], $row['rArea'],  $row['sano'], $row['hp'], $regflag,
