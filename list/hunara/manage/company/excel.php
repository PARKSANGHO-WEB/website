...
try {
  ...
  $cnt = 0;
    for ($i = 2 ; $i <= $maxRow ; $i++) {
               ...
               $team = $objWorksheet->getCell('B' . $i)->getValue(); // B열
      ...
      $duplicate = 'N';
        ...
        if($mem_row2[sano]){
          ...
          $duplicate = 'Y';
			  ...
			  $qry = "insert into tb_employee_excel(name,team,class,digit7,sano,weight,tel,hp,email,cdx,max,wdate,duplicate) values ('".$name."','".$team."','".$class."','".$digit7."','".$sano."','".$weight."','".$tel."','".$hp."','".$email."','".$bkind."','".$max."',now(),'".$duplicate."')";
