...
<? 
    ...
    if($mode == "CHUCHUM"){
        ...
        for($ii=0; $ii<sizeof($midxArr); $ii++){
            $keys = explode(":",$midxArr[$ii]);
            $midx = $keys[0];
            $seq = $keys[1];
			
			if($ii == (sizeof($midxArr)-1)){
				$midx_arr .= $midx;
				$seq_arr .= $seq;
			} else {
				$midx_arr .= $midx.",";
            ...
            for ($h=1; $h < 4; $h++){ //1지망 ~ 3지망
                ...
                */
                ...
                $sql .= "   and a.seq = d.seq     ";    
                $sql .= "   and a.midx in (".$midx_arr.")  ";
                $sql .= "   and a.seq in (".$seq_arr.")    ";
                $sql .= "   and c.del_yn = 'N'    ";
                $sql .= " )t ";   
        ...
        for($iii=0; $iii < count($midxArr); $iii++){
            $midx = $midxArr[$iii];

            //마감처리 완료
            $sql99 = " update tb_reInfo set hide_yn = 'Y' ";
            $sql99 .= " where midx = '{$midx}'           ";

            $rs99 = mysqli_query($gconnet,$sql99); 

            $sql98 = " update tb_comhuM set hide_yn = 'Y' ";
            $sql98 .= " where midx = '{$midx}'           ";

            $rs98 = mysqli_query($gconnet,$sql98); 
        ...
        for($iiii=0; $iiii < count($midxArr); $iiii++){
            $midx = $midxArr[$iiii];

            //마감처리 완료
            $sql99 = " update tb_reInfo set hide_yn = 'N' ";
            $sql99 .= " where midx = '{$midx}'           ";

            $rs99 = mysqli_query($gconnet,$sql99); 

            $sql98 = " update tb_comhuM set hide_yn = 'N' ";
            $sql98 .= " where midx = '{$midx}'           ";
