...
<?php
$bkind = trim(sqlfilter($_REQUEST['id4']));
if($bkind == ''){
...
//var_dump($_FILES);
if (empty($_FILES['excelFile3']['name'])){
...
$fname = explode('.',$_FILES['excelFile3']['name']);
...
echo 'output:'.basename($_FILES['excelFile3']['tmp_name']);

if(move_uploaded_file($_FILES['excelFile3']['tmp_name'], $uploadfile)) {
...
$filename = $_FILES['excelFile3']['tmp_name']; // 읽어들일 엑셀 파일의 경로와 파일명을 지정한다.
