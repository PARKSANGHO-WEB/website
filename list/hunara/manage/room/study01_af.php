...
for($i=0; $i<$_POST['image_count2']; $i++) {
    $j = $i +1;
	...
	if(isset($_FILES[$image_id]) && !$_FILES[$image_id]['error']) {
		if(in_array($_FILES[$image_id]['type'], $imageKind)) {
			if(move_uploaded_file($_FILES[$image_id]['tmp_name'], $dir.$image_file)) {
                ...
                $query_file = " update tb_hu set "; 
                $query_file .= " thum_img = '".$lo."' ";
...
for($i=0; $i<$_POST['image_count3']; $i++) {
    $j = $i +1;
...
for($i=0; $i<$_POST['image_count4']; $i++) {
    $j = $i +1;
...
for($i=0; $i<$_POST['image_count5']; $i++) {
    $j = $i +1;
