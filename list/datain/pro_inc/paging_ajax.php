	...
	//echo "시작페이지 = ".$start_page." | 현재페이지 = ".$c_page." || current_page = ".$current_page." ||| 종료페이지 = ".$end_page." |||| 총 페이지수 = ".$total_page."<br><br>";
			...
			###페이지 출력
				for ($i=$start_page;$i <= $end_page;$i++){
					...
					?>
						<a href="javascript:get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$page?>');"><?=$i?></a>
					<?
					}else{
					?>
						<a href="javascript:get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$page?>');" class="on"><?=$i?></a>
					<?
					}
...
;					}
			...
			###다음버튼
				if($total_page > $end_page){
					...
					?>
						<a href="get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$next_list?>');" class="next">다음 페이지</a>
					<?
					// 처음
