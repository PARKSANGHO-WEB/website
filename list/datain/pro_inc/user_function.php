...
function get_calcurate_point($satisfaction_option_idx,$orgscore){
	...
	$satisfaction_option_scorepoint2 = $row['analysis_report_satisfaction_option_scorepoint2'];
...
function get_calcurate_point_service($service_option_idx,$orgscore){
	...
	$service_option_scorepoint = $row['analysis_report_service_option_scorepoint'];
	$service_option_scorepoint2 = $row['analysis_report_service_option_scorepoint2'];
...
function get_satisf_csi_avg($satisfaction_option_idx,$satisfaction_option_100data_idx,$data_no){
	...
	$compet_info_sql = "select analysis_report_idx,analysis_report_satisfaction_option_scorepoint,analysis_report_satisfaction_option_scorepoint2 from wise_analysis_report_satisfaction_option where 1 and idx='".$satisfaction_option_idx."'";
	$compet_info_query = mysqli_query($GLOBALS['gconnet'],$compet_info_sql);
	$row = mysqli_fetch_array($compet_info_query);
	...
	$model_x_per = round(1/$query_file_1_cnt,2); // 각 독립변수 별 가중치 %  
	
	$csi_data_tot = 0;
	for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){ // 독립변수 루프 시작 
		$row_file_1 = mysqli_fetch_array($query_file_1);
			
		$sql_file_2 = "select idx,(select quiz_no from wise_analysis_quiz where 1 and idx=a.analysis_report_satisfaction_option_factor_no) as quiz_no from wise_analysis_report_satisfaction_option_model_quiz a where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and satisfaction_option_model_idx='".$row_file_1['idx']."' order by idx asc"; 
		$query_file_2 = mysqli_query($GLOBALS['gconnet'],$sql_file_2);
		$query_file_2_cnt = mysqli_num_rows($query_file_2);
		
		$csi_data_1 = 0;
		for($opt_i2=0; $opt_i2<$query_file_2_cnt; $opt_i2++){ // 독립변수별 문항루프 시작 
			$row_file_2 = mysqli_fetch_array($query_file_2);
			
			$sql_file_3 = "select analysis_report_satisfaction_100data_quiz_val from wise_analysis_report_satisfaction_100data_detail where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and analysis_report_satisfaction_100data_no='".$data_no."' and analysis_report_satisfaction_100data_quiz_no='".$row_file_2['quiz_no']."'"; 
			$query_file_3 = mysqli_query($GLOBALS['gconnet'],$sql_file_3);
			$row_file_3 = mysqli_fetch_array($query_file_3);
			$csi_data_1 = $csi_data_1+$row_file_3['analysis_report_satisfaction_100data_quiz_val']; // 문항별 점수합계  
		}  // 독립변수별 문항루프 종료 
		
		$csi_data_2 = round($csi_data_1/$query_file_2_cnt,$satisfaction_option_scorepoint); // 독립변수 합계 평균 
		$csi_data_2 = $csi_data_2 * $model_x_per; // 평균에 가중치 곱 
		$csi_data_tot = $csi_data_tot + $csi_data_2;
	} // 독립변수 루프 종료 
}
...
// 다면평가 시작 
function get_calcurate_point_damyun($damyun_option_idx,$orgscore){
	$memid = $_SESSION['wiz_session']['id'];
	$subid = $_SESSION['wiz_session']['subid'];

	$compet_info_sql = "select analysis_report_damyun_option_score,analysis_report_damyun_option_percent,analysis_report_damyun_option_scorepoint,analysis_report_damyun_option_scorepoint2 from wise_analysis_report_damyun_option where 1 and idx='".$damyun_option_idx."'";
	//echo $compet_info_sql."<br>";
	if($memid){
		$compet_info_sql .= " and memid='".$memid."'";
	}
	...
	$damyun_option_scorepoint = $row['analysis_report_damyun_option_scorepoint'];
	$damyun_option_scorepoint2 = $row['analysis_report_damyun_option_scorepoint2'];
	
	if(!$orgscore){
		$calcurate_point = 0;
	} else {
		if($damyun_option_percent=="1"){ // 원점수 그대로 
			$calcurate_point = $orgscore;
		} else { // 100 점 변환 
			$calcurate_point_1 = $damyun_option_score-1; // 만점 -1 
			if(($orgscore-1) <= 0){
				//$calcurate_point_2 = $orgscore/$calcurate_point_1;
				$calcurate_point_2 = ($orgscore-1)/$calcurate_point_1;
			} else {
			...
			$calcurate_point = $calcurate_point_2*100;
			$calcurate_point = round($calcurate_point,$damyun_option_scorepoint);
		} // 구분 종료 
	}
...
function get_calcurate_point_damyun_samescyn_quiz($damyun_option_idx,$analysis_report_idx,$quizno,$orgscore){
	
	//$sql_file_2 = "select analysis_report_damyun_option_samescyn_quiz_score from wise_analysis_report_damyun_option_samescyn_quiz where 1 and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_option_samescyn_quiz_no='".$quizno."'";

	$sql_file_2 = "select analysis_report_damyun_option_samescyn_quiz_score from wise_analysis_report_damyun_option_samescyn_quiz where 1 and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_option_samescyn_quiz_no in (select idx from wise_analysis_quiz where 1 and quiz_no='".$quizno."' and analysis_idx='".$analysis_report_idx."')";
	//echo "sql_file_2 = ".$sql_file_2."<br>";
	$query_file_2 = mysqli_query($GLOBALS['gconnet'],$sql_file_2);
	$query_file_2_cnt = mysqli_num_rows($query_file_2);
	...
	if($query_file_2_cnt == 0){
		...
		$row_file_2 = mysqli_fetch_array($query_file_2);
		$bdscore = $row_file_2['analysis_report_damyun_option_samescyn_quiz_score']/100; // 배당 % 화 
		//echo "orgscore = ".$orgscore."<br>";
		//echo "samescyn_quiz_score = ".$row_file_2['analysis_report_damyun_option_samescyn_quiz_score']."<br>";
		//echo "bdscore = ".$bdscore."<br>";
		$caled_sc = $orgscore*$bdscore;
	}
	
	return $caled_sc;
}

function get_calcurate_point_damyun_samescyn($damyun_option_idx,$damyun_100data_no,$damyun_option_samescyn_idx){

	/*$sql_file_1 = "select idx,(select quiz_no from wise_analysis_quiz where 1 and idx=wise_analysis_report_damyun_option_samescyn_quiz.analysis_report_damyun_option_samescyn_quiz_no) as quiz_no from wise_analysis_report_damyun_option_samescyn_quiz where 1 and damyun_option_idx='".$damyun_option_idx."' and damyun_option_samescyn_idx='".$damyun_option_samescyn_idx."' order by idx asc";
	echo "sql_file_1 = ".$sql_file_1."<br>";
	$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
	$query_file_1_cnt = mysqli_num_rows($query_file_1);
	...
	for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){ 
		...
		$sql_file_2 = "select analysis_report_damyun_100data_quiz_val from wise_analysis_report_damyun_100data_detail where 1 and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_100data_quiz_no='".$row_file_1['quiz_no']."' and analysis_report_damyun_100data_no='".$damyun_100data_no."' and analysis_report_damyun_100data_group_no='0'";
		echo "sql_file_2 = ".$sql_file_2."<br>";
		$query_file_2 = mysqli_query($GLOBALS['gconnet'],$sql_file_2);
		$row_file_2 = mysqli_fetch_array($query_file_2);

		$sql_file_3 = "select analysis_report_damyun_100data_quiz_val from wise_analysis_report_damyun_100data_detail where 1 and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_100data_quiz_no='".$row_file_1['quiz_no']."' and analysis_report_damyun_100data_no='".$damyun_100data_no."' and (analysis_report_damyun_100data_group_no !='0' or analysis_report_damyun_100data_group_no is null)";
		echo "sql_file_3 = ".$sql_file_3."<br><br>";
		$query_file_3 = mysqli_query($GLOBALS['gconnet'],$sql_file_3);
		$row_file_3 = mysqli_fetch_array($query_file_3);
				
		$bdscore = $row_file_2['analysis_report_damyun_100data_quiz_val']*$row_file_3['analysis_report_damyun_100data_quiz_val']; 
		$tot_score = $tot_score+$bdscore;
	}*/ 
	//echo "tot_score = ".$tot_score."<br>";
	//exit;

	$sql_file_2 = "select sum(analysis_report_damyun_100data_quiz_val) as sumval from wise_analysis_report_damyun_100data_detail where 1 and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_100data_quiz_no in (select quiz_no from wise_analysis_quiz where 1 and idx in (select analysis_report_damyun_option_samescyn_quiz_no from wise_analysis_report_damyun_option_samescyn_quiz where 1 and damyun_option_idx='".$damyun_option_idx."' and damyun_option_samescyn_idx='".$damyun_option_samescyn_idx."')) and analysis_report_damyun_100data_no='".$damyun_100data_no."' and analysis_report_damyun_100data_group_no='0'";
	//echo "sql_file_2 = ".$sql_file_2."<br>";
	$query_file_2 = mysqli_query($GLOBALS['gconnet'],$sql_file_2);
	$row_file_2 = mysqli_fetch_array($query_file_2);

	$sql_file_3 = "select sum(analysis_report_damyun_100data_quiz_val) as sumval from wise_analysis_report_damyun_100data_detail where 1 and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_100data_quiz_no in (select quiz_no from wise_analysis_quiz where 1 and idx in (select analysis_report_damyun_option_samescyn_quiz_no from wise_analysis_report_damyun_option_samescyn_quiz where 1 and damyun_option_idx='".$damyun_option_idx."' and damyun_option_samescyn_idx='".$damyun_option_samescyn_idx."')) and analysis_report_damyun_100data_no='".$damyun_100data_no."' and (analysis_report_damyun_100data_group_no !='0' or analysis_report_damyun_100data_group_no is null)";
	//echo "sql_file_3 = ".$sql_file_3."<br>";
	$query_file_3 = mysqli_query($GLOBALS['gconnet'],$sql_file_3);
	$row_file_3 = mysqli_fetch_array($query_file_3);

	$tot_score = $row_file_2['sumval']*$row_file_3['sumval']; 

	//echo "sumval 1 = ".$row_file_2['sumval']."<br>";
	//echo "sumval 2 = ".$row_file_3['sumval']."<br><br>";

	return $tot_score;
}

function get_calcurate_point_damyun_samescyn_total($damyun_option_idx,$query_file_2_cnt,$damyun_100data_no){
		
	/*$tot_score = 0;
	for($opt_i=0; $opt_i<$query_file_2_cnt; $opt_i++){ 
		$row_file_2 = mysqli_fetch_array($query_file_2);
		//$bdscore = get_calcurate_point_damyun_samescyn($damyun_option_idx,$row_file_2['idx']);

		$sql_file_3 = "select analysis_report_damyun_100data_quiz_val from wise_analysis_report_damyun_100data_detail where 1 and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_100data_model_no='".$row_file_2['idx']."' and analysis_report_damyun_100data_no='".$damyun_100data_no."'";
		//echo "sql_file_3 = ".$sql_file_3."<br>";
		$query_file_3 = mysqli_query($GLOBALS['gconnet'],$sql_file_3);
		$row_file_3 = mysqli_fetch_array($query_file_3);

		$bdscore = $row_file_3['analysis_report_damyun_100data_quiz_val'];

		$tot_score = $tot_score+$bdscore;
	}*/

	$sql_file_3 = "select sum(analysis_report_damyun_100data_quiz_val) as sumval from wise_analysis_report_damyun_100data_detail where 1 and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_100data_model_no in (select idx from wise_analysis_report_damyun_option_samescyn where 1 and damyun_option_idx='".$damyun_option_idx."' and (analysis_report_damyun_option_samescyn_title != '' or analysis_report_damyun_option_samescyn_title is null)) and analysis_report_damyun_100data_no='".$damyun_100data_no."'";
	//echo "sql_file_3 = ".$sql_file_3."<br>";
	$query_file_3 = mysqli_query($GLOBALS['gconnet'],$sql_file_3);
	$row_file_3 = mysqli_fetch_array($query_file_3);
	$tot_score = $row_file_3['sumval'];

	if($query_file_2_cnt == 0){
		$tot_score_tot = 0;
	} else {
		$tot_score_tot = $tot_score/$query_file_2_cnt;
	}

	//echo "tot_score = ".$tot_score_tot."<br>";
	//exit;
	return $tot_score_tot;
}

function get_damyun_totrate($tot_point){
	if($tot_point < 70){
		$tot_rate = "불량";
	} elseif($tot_point >= 70 && $tot_point < 80){
		$tot_rate = "미흡";
...
function get_damyun_100data_no_val($damyun_option_idx,$data_no,$file_1_idx2="",$file_1_group2="",$file_1_title2){
	$sql_data_2 = "select analysis_report_damyun_100data_quiz_val from wise_analysis_report_damyun_100data_detail where 1 and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_100data_no='".$data_no."' and analysis_report_damyun_100data_group_no = '0'";
	
	if($file_1_group2){
		$sql_data_2 .= " and analysis_report_damyun_100data_group_no='".$file_1_group2."'"; 
	} elseif($file_1_idx2){
		$sql_data_2 .= " and analysis_report_damyun_100data_quiz_no='".$file_1_idx2."'"; 
	}
	/*if($file_1_title2){
		$sql_data_2 .= " and analysis_report_damyun_100data_quiz_title='".$file_1_title2."'"; 
		//echo $sql_data_2."<br>";
	}*/
	$query_data_2 = mysqli_query($GLOBALS['gconnet'],$sql_data_2);
	$row_data_2 = mysqli_fetch_array($query_data_2);

	return $row_data_2['analysis_report_damyun_100data_quiz_val'];
}

function get_damyun_100data_tlc_val($damyun_option_idx,$data_no,$col_name){
	$sql_data_2 = "select ".$col_name." as colval from wise_analysis_report_damyun_100data_tlc where 1 and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_100data_no='".$data_no."'";
	$query_data_2 = mysqli_query($GLOBALS['gconnet'],$sql_data_2);
	$row_data_2 = mysqli_fetch_array($query_data_2);
...
function get_damyun_gquiz_val($data_no,$analysis_idx,$quiz_no){
	$opt_sql = "select data_val from wise_analysis_data where 1 and analysis_idx='".$analysis_idx."' and data_no='".$data_no."' and quiz_no='".$quiz_no."'";
	//echo "service_gquiz_val = ".$opt_sql."<br>";
	$opt_query = mysqli_query($GLOBALS['gconnet'],$opt_sql);
	$opt_row = mysqli_fetch_array($opt_query);
...
function set_rst_damyun_tlc($damyun_option_idx,$damyun_option_100data_idx,$damyun_100data_no,$analysis_report_idx){
	$sql_file_1 = "select analysis_report_damyun_100data_quiz_no,analysis_report_damyun_100data_quiz_val from wise_analysis_report_damyun_100data_detail where 1 and damyun_option_idx='".$damyun_option_idx."' and damyun_option_100data_idx='".$damyun_option_100data_idx."' and analysis_report_damyun_100data_no='".$damyun_100data_no."' and analysis_report_damyun_100data_quiz_title = '총계' order by analysis_report_damyun_100data_quiz_no asc";
	//echo "sql_file_1 = ".$sql_file_1."<br>"; exit;
	$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
	$query_file_1_cnt = mysqli_num_rows($query_file_1);
	...
	for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){ // 응답자의 참여데이터 루프 시작 
		...
		$sql_file_2 = "select analysis_report_damyun_option_samescyn_quiz_score from wise_analysis_report_damyun_option_samescyn_quiz where 1 and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_option_samescyn_quiz_no=(select idx from wise_analysis_quiz where 1 and quiz_no='".$row_file_1['analysis_report_damyun_100data_quiz_no']."' and analysis_idx='".$analysis_report_idx."')";
		...
		for($opt2_i=0; $opt2_i<$query_file_2_cnt; $opt2_i++){ // 문항 배점배당 루프 시작 
			...
			//$bdscore = $row_file_2['analysis_report_damyun_option_samescyn_quiz_score']/100; // 배당 % 화 
			//$tot_sub_score = $tot_sub_score+($row_file_1['analysis_report_damyun_100data_quiz_val']*$bdscore); // 각 문항당 배점적용 계산값

			$tot_sub_score = $tot_sub_score+$row_file_1['analysis_report_damyun_100data_quiz_val']; // 각 문항당 배점적용 계산값
	...
	$row_file_1 = mysqli_fetch_array($query_file_1);
	$tot_score = $row_file_1['analysis_report_damyun_100data_quiz_val'];

	$query_in = "insert into wise_analysis_report_damyun_100data_tlc set"; 
	...
	$query_in .= " analysis_report_damyun_100data_total = '".$tot_score."', ";
	$query_in .= " wdate = now() ";

	$result_in = mysqli_query($GLOBALS['gconnet'],$query_in);
}

function set_rst_damyun_tlc_align($damyun_option_idx,$damyun_option_100data_idx){
	//$sql_file_1 = "select idx from wise_analysis_report_damyun_100data_tlc where 1 and damyun_option_idx='".$damyun_option_idx."' and damyun_option_100data_idx='".$damyun_option_100data_idx."' order by CONVERT(analysis_report_damyun_100data_total, UNSIGNED) desc";

	$sql_file_1 = "select idx from wise_analysis_report_damyun_100data_detail where 1 and damyun_option_idx='".$damyun_option_idx."' and damyun_option_100data_idx='".$damyun_option_100data_idx."' and analysis_report_damyun_100data_quiz_title = '총계' order by CONVERT(analysis_report_damyun_100data_quiz_val, UNSIGNED) desc";
	//echo $sql_file_1."<br>";
	$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
	$query_file_1_cnt = mysqli_num_rows($query_file_1);
	...
	for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){ // 총점 데이터 루프 시작 
		...
		//$query_in = "update wise_analysis_report_damyun_100data_tlc set"; 
		$query_in = "update wise_analysis_report_damyun_100data_detail set"; 
		$query_in .= " analysis_report_damyun_100data_count = '".($opt_i+1)."' ";
		$query_in .= " where 1 and idx='".$row_file_1['idx']."'";
		//echo $query_in."<br>";
		$result_in = mysqli_query($GLOBALS['gconnet'],$query_in);
		//exit;
...
function get_damyun_data_val_avg($damyun_option_idx,$analysis_report_idx,$quiz_no,$tquiz_no,$tanswer){
	...
	$sql_prev_2 = "select idx from wise_analysis_report_damyun_option_point_case where 1 and damyun_option_idx='".$damyun_option_idx."'"; 
	$query_prev_2 = mysqli_query($GLOBALS['gconnet'],$sql_prev_2);
	$query_prev_2_cnt = mysqli_num_rows($query_prev_2);
	if($query_prev_2_cnt > 0){
		//$detail_tb_name = "wise_analysis_report_damyun_100data_detail_case";
		$detail_tb_name = "wise_analysis_report_damyun_100data_detail";
	} else {
		$detail_tb_name = "wise_analysis_report_damyun_100data_detail";
	}

	$opt_sql .= "select analysis_report_damyun_100data_quiz_val from ".$detail_tb_name." where 1 and analysis_report_damyun_100data_group_no = '0' and analysis_report_damyun_100data_group_no = '0' and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_100data_quiz_no='".$quiz_no."' and analysis_report_damyun_100data_no in (select data_no from wise_analysis_data where 1 and analysis_idx='".$analysis_report_idx."' and quiz_no='".$tquiz_no."' and data_val='".$tanswer."')";
	...
	if($opt_cnt == 0){
		...
		for($opt_i2=0; $opt_i2<$opt_cnt; $opt_i2++){  
			...
			$damyun_100data_quiz_val = $opt_row['analysis_report_damyun_100data_quiz_val'];		
			$total_point = $total_point + $damyun_100data_quiz_val;
			//echo "per = ".$damyun_100data_quiz_val."<br>";
			//echo "total = ".$total_point."<br>";
		}
	
		$avg_point = round($total_point/$opt_cnt,$damyun_option_scorepoint);
...
function get_damyun_data_val_avg2($damyun_option_idx,$analysis_report_idx,$quiz_no){
	...
	$opt_sql .= "select analysis_report_damyun_100data_quiz_val from ".$detail_tb_name." where 1 and analysis_report_damyun_100data_group_no = '0' and analysis_report_damyun_100data_group_no = '0' and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_100data_quiz_no='".$quiz_no."'";
	$opt_query = mysqli_query($GLOBALS['gconnet'],$opt_sql);
	$opt_cnt = mysqli_num_rows($opt_query);
...
function get_damyun_data_val_avg_tot1($damyun_option_idx,$quiz_no){
	
	$opt_sql .= "select analysis_report_damyun_data_quiz_val from wise_analysis_report_damyun_data_quiz where 1 and damyun_option_idx = '".$damyun_option_idx."' and analysis_report_damyun_data_quiz_no = '".$quiz_no."' and analysis_report_damyun_data_quiz_title='전체'";
	$opt_query = mysqli_query($GLOBALS['gconnet'],$opt_sql);
	$opt_cnt = mysqli_num_rows($opt_query);
	...
	for($opt_i2=0; $opt_i2<$opt_cnt; $opt_i2++){  
		$opt_row = mysqli_fetch_array($opt_query);
		$total_point = $total_point + $opt_row['analysis_report_damyun_data_quiz_val'];
	}
...
function get_damyun_data_val_avg_tot2($damyun_option_idx,$quiz_no,$ans_i){
	
	$opt_sql .= "select analysis_report_damyun_data_quiz_val from wise_analysis_report_damyun_data_quiz where 1 and damyun_option_idx = '".$damyun_option_idx."' and analysis_report_damyun_data_quiz_no = '".$quiz_no."' and analysis_report_damyun_data_quiz_title='".$ans_i." 의 평균'";
	$opt_query = mysqli_query($GLOBALS['gconnet'],$opt_sql);
	$opt_cnt = mysqli_num_rows($opt_query);
...
function set_rst_damyun_data_quiz_align($damyun_option_idx,$damyun_option_100data_idx){
	$sql_file_1 = "select idx from wise_analysis_report_damyun_100data_tlc where 1 and damyun_option_idx='".$damyun_option_idx."' and damyun_option_100data_idx='".$damyun_option_100data_idx."' order by CONVERT(analysis_report_damyun_100data_total, UNSIGNED) desc";
	$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
	$query_file_1_cnt = mysqli_num_rows($query_file_1);
...
function get_report_damyun_data_quiz_val($damyun_option_idx,$damyun_data_idx,$where=""){
	
	$opt_sql = "select analysis_report_damyun_data_quiz_val from wise_analysis_report_damyun_data_quiz where damyun_option_idx='".$damyun_option_idx."' and  damyun_data_idx='".$damyun_data_idx."'";
	if($where){
		$opt_sql .= $where;		
	}
	...
	return $opt_row['analysis_report_damyun_data_quiz_val'];
}

function set_report_damyun_data_quiz_val($damyun_option_idx,$file_1_quizno2,$title,$val){

	//$opt_sql = "select idx from wise_analysis_report_damyun_data_quiz where 1 and damyun_option_idx = '".$damyun_option_idx."' and analysis_report_damyun_data_quiz_no = '".$file_1_quizno2."' and damyun_data_idx is null";
	$opt_sql = "select idx from wise_analysis_report_damyun_data_quiz where 1 and damyun_option_idx = '".$damyun_option_idx."' and analysis_report_damyun_data_quiz_no = '".$file_1_quizno2."' and analysis_report_damyun_data_quiz_title = '".$title."' and damyun_data_idx is null";
	$opt_query = mysqli_query($GLOBALS['gconnet'],$opt_sql);
	...
	if(mysqli_num_rows($opt_query) == 0){
		...
		$query_in3 .= " analysis_report_damyun_data_quiz_title = '".$title."', ";
		$query_in3 .= " analysis_report_damyun_data_quiz_val = '".$val."', ";
		$query_in3 .= " wdate = now() ";
		$result_in3 = mysqli_query($GLOBALS['gconnet'],$query_in3);

		$sql_file_1 = "select idx from wise_analysis_report_damyun_data_quiz where 1 and damyun_option_idx = '".$damyun_option_idx."' and analysis_report_damyun_data_quiz_no = '".$file_1_quizno2."' and damyun_data_idx is null order by analysis_report_damyun_data_quiz_val desc";
		$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
		$query_file_1_cnt = mysqli_num_rows($query_file_1);
		...
		for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){ 
			...
			$query_in = "update wise_analysis_report_damyun_data_quiz set"; 
			$query_in .= " analysis_report_damyun_data_quiz_align = '".($opt_i+1)."' ";
			$query_in .= " where 1 and idx='".$row_file_1['idx']."'";
			$result_in = mysqli_query($GLOBALS['gconnet'],$query_in);
		} 

	} else {
		$query_in3 = "update wise_analysis_report_damyun_data_quiz set"; 
		$query_in3 .= " analysis_report_damyun_data_quiz_val = '".$val."'";
		$query_in3 .= " where 1 and damyun_option_idx = '".$damyun_option_idx."' and analysis_report_damyun_data_quiz_no = '".$file_1_quizno2."' and damyun_data_idx is null";
		//$result_in3 = mysqli_query($GLOBALS['gconnet'],$query_in3);
	}
	
}

function set_report_damyun_data_total_align($damyun_option_idx,$analysis_report_idx){
	$sql_file_1 = "select idx,analysis_report_damyun_option_percent,analysis_report_damyun_option_data_no,analysis_report_damyun_option_data_title from wise_analysis_report_damyun_data where 1 and damyun_option_idx='".$damyun_option_idx."'"; // 일반문항 
	$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
	$query_file_1_cnt = mysqli_num_rows($query_file_1);
	...
	$sql_file_2 = "select idx,analysis_report_damyun_option_percent,analysis_report_damyun_option_data_no,analysis_report_damyun_option_data_title from wise_analysis_report_damyun_data where 1 and damyun_option_idx='".$damyun_option_idx."'"; // 집단문항 
	$query_file_2 = mysqli_query($GLOBALS['gconnet'],$sql_file_2);
	$query_file_2_cnt = mysqli_num_rows($query_file_2);
	...
	for($opt4_i=0; $opt4_i<$query_file_4_cnt; $opt4_i++){ // 집단구분 보기 시작 
		...
		set_report_damyun_data_quiz_val($damyun_option_idx,$row_file_4['analysis_report_damyun_data_quiz_no'],$row_file_4['analysis_report_damyun_data_quiz_title']."총계",$row_tot_avg); // 총점입력 및 순위조정 

	} // 집단구분 보기 종료
}

function set_rst_damyun_tlc_case($damyun_option_idx,$damyun_option_100data_idx,$damyun_100data_no,$analysis_report_idx){
	$sql_file_1 = "select analysis_report_damyun_100data_quiz_no,analysis_report_damyun_100data_quiz_val from wise_analysis_report_damyun_100data_detail_case where 1 and analysis_report_damyun_100data_group_no = '0' and damyun_option_idx='".$damyun_option_idx."' and damyun_option_100data_idx='".$damyun_option_100data_idx."' and analysis_report_damyun_100data_no='".$damyun_100data_no."' and analysis_report_damyun_100data_quiz_title = '총계' order by analysis_report_damyun_100data_quiz_no asc";
	//echo "sql_file_1 = ".$sql_file_1."<br>"; exit;
	$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
	$query_file_1_cnt = mysqli_num_rows($query_file_1);
	...
	$query_in = "insert into wise_analysis_report_damyun_100data_tlc_case set"; 
	$query_in .= " damyun_option_idx = '".$damyun_option_idx."', ";
	$query_in .= " damyun_option_100data_idx = '".$damyun_option_100data_idx."', ";
	$query_in .= " analysis_report_damyun_100data_no = '".$damyun_100data_no."', ";
	$query_in .= " analysis_report_damyun_100data_total = '".$tot_score."', ";
...
function get_frequency_val_damyun($damyun_option_idx,$analysis_report_idx,$quiz_no="",$ans_no="",$level_idx=""){
	$sql_prev_2 = "select idx from wise_analysis_report_damyun_option_point_case where 1 and damyun_option_idx='".$damyun_option_idx."'"; 
	$query_prev_2 = mysqli_query($GLOBALS['gconnet'],$sql_prev_2);
	$query_prev_2_cnt = mysqli_num_rows($query_prev_2);
	if($query_prev_2_cnt > 0){
		$detail_tb_name = "wise_analysis_report_damyun_100data_detail_case";
		$detail_tb2_name = "wise_analysis_report_damyun_100data_tlc_case";
	} else {
		$detail_tb_name = "wise_analysis_report_damyun_100data_detail";
		$detail_tb2_name = "wise_analysis_report_damyun_100data_tlc";
	}
	
	if($quiz_no && $ans_no){
		$opt_sql = "select distinct(analysis_report_damyun_100data_no) from ".$detail_tb_name." where 1 and analysis_report_damyun_100data_group_no = '0' and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_100data_no in (select data_no from wise_analysis_data where 1 and analysis_idx='".$analysis_report_idx."' and quiz_no='".$quiz_no."' and data_val='".$ans_no."')";
	} else {
		$opt_sql = "select distinct(analysis_report_damyun_100data_no) from ".$detail_tb_name." where 1 and analysis_report_damyun_100data_group_no = '0' and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_100data_no in (select data_no from wise_analysis_data where 1 and analysis_idx='".$analysis_report_idx."')";
	}

	if($level_idx){
		$opt_sql .= " and analysis_report_damyun_100data_no in (select analysis_report_damyun_100data_no from ".$detail_tb2_name." where 1 and analysis_report_damyun_option_level_no='".$level_idx."')";
	}
...
function get_ratio_val_damyun($damyun_option_idx,$analysis_report_idx,$quiz_no,$ans_no,$level_idx=""){
	$compet_info_sql = "select analysis_report_damyun_option_scorepoint from wise_analysis_report_damyun_option where 1 and idx='".$damyun_option_idx."'";
	$compet_info_query = mysqli_query($GLOBALS['gconnet'],$compet_info_sql);
	$compet_info_row = mysqli_fetch_array($compet_info_query);
	$damyun_option_scorepoint = $compet_info_row['analysis_report_damyun_option_scorepoint'];
	...
	$opt_tot_sql = "select distinct(analysis_report_damyun_100data_no) from ".$detail_tb_name." where 1 and analysis_report_damyun_100data_group_no = '0' and damyun_option_idx='".$damyun_option_idx."'";
	if($level_idx){
		$opt_tot_sql .= " and analysis_report_damyun_100data_no in (select analysis_report_damyun_100data_no from ".$detail_tb2_name." where 1 and analysis_report_damyun_option_level_no='".$level_idx."')";
	}
	$opt_tot_query = mysqli_query($GLOBALS['gconnet'],$opt_tot_sql);
	$opt_tot_cnt = mysqli_num_rows($opt_tot_query);
	...
	if($opt_tot_cnt == 0){
		...
		$avg_point = round($opt_cnt/$opt_tot_cnt,$damyun_option_scorepoint);
		$avg_point = $avg_point*100;
	}
...
// 의사결정모델 보고서 시작 
function get_calcurate_point_decision($decision_option_idx,$orgscore){
	$memid = $_SESSION['wiz_session']['id'];
	$subid = $_SESSION['wiz_session']['subid'];

	$compet_info_sql = "select analysis_report_decision_option_scorepoint,analysis_report_decision_option_scorepoint2,analysis_report_decision_option_calpath,analysis_report_decision_option_calpath2,analysis_report_decision_option_case,analysis_report_decision_option_case_no from wise_analysis_report_decision_option where 1 and idx='".$decision_option_idx."'";
	//echo $compet_info_sql."<br>";
	if($memid){
		$compet_info_sql .= " and memid='".$memid."'";
	}
	...
	$decision_option_scorepoint = $row['analysis_report_decision_option_scorepoint'];
	$decision_option_scorepoint2 = $row['analysis_report_decision_option_scorepoint2'];
...
function get_decision_100data_no_val($decision_option_idx,$data_no,$quiz_no,$file_1_idx2="",$file_1_group2="",$file_1_title2=""){
	$sql_data_2 = "select analysis_report_decision_100data_quiz_val from wise_analysis_report_decision_100data_detail where 1 and decision_option_idx='".$decision_option_idx."' and analysis_report_decision_100data_no='".$data_no."' and analysis_report_decision_100data_quiz_no='".$quiz_no."'";
	
	if($file_1_group2){
		$sql_data_2 .= " and analysis_report_decision_100data_group_no='".$file_1_group2."'"; 
	} elseif($file_1_idx2){
		$sql_data_2 .= " and analysis_report_decision_100data_quiz_no='".$file_1_idx2."'"; 
	}
	/*if($file_1_title2){
		$sql_data_2 .= " and analysis_report_decision_100data_quiz_title='".$file_1_title2."'"; 
		//echo $sql_data_2."<br>";
	}*/
	$query_data_2 = mysqli_query($GLOBALS['gconnet'],$sql_data_2);
	$row_data_2 = mysqli_fetch_array($query_data_2);

	return $row_data_2['analysis_report_decision_100data_quiz_val'];
}

function get_decision_100data2_no_val($decision_option_idx,$data_no,$quiz_no,$file_1_idx2="",$file_1_group2="",$file_1_title2=""){
	$sql_data_2 = "select analysis_report_decision_100data_quiz_val from wise_analysis_report_decision_100data_detail2 where 1 and decision_option_idx='".$decision_option_idx."' and analysis_report_decision_100data_no='".$data_no."' and analysis_report_decision_100data_quiz_no='".$quiz_no."'";
...
function get_decision_100data3_no_val($decision_option_idx,$data_no,$quiz_no,$file_1_idx2="",$file_1_group2="",$file_1_title2=""){
	$sql_data_2 = "select analysis_report_decision_100data_quiz_val from wise_analysis_report_decision_100data_detail3 where 1 and decision_option_idx='".$decision_option_idx."' and analysis_report_decision_100data_no='".$data_no."' and analysis_report_decision_100data_quiz_no='".$quiz_no."'";
...
function get_report_type($rtype){ 
	switch ($rtype) {
		case "manjok" : 
		$rtype_name = "만족도모델 보고서";
		break;
		case "service" : 
		$rtype_name = "서비스평가모델 보고서";
		break;
		case "damyun" : 
		$rtype_name = "다면평가모델 보고서";
		break;
		case "decision" : 
		$rtype_name = "의사결정모델 보고서";
		break;
	}
	return $rtype_name;
}

function get_option_url($rtype,$ridx){
	switch ($rtype) {
		case "manjok" : 
		$rtype_name = "/report/manjok_1.php?satisfaction_option_idx=".$ridx."";
		break;
		case "service" : 
		$rtype_name = "/report/service_1.php?service_option_idx=".$ridx."";
		break;
		case "damyun" : 
		$rtype_name = "/report/damyun_1.php?damyun_option_idx=".$ridx."";
		break;
		case "decision" : 
		$rtype_name = "/report/decision_1.php?decision_option_idx=".$ridx."";
		break;
	}
	return $rtype_name;
}
