...
?>
	...
	<?}?>
		...
		?>
			...
			<td><?=round($data_tot_1/sizeof($file_1_title_arr),$service_option_scorepoint)?></td>
	<?if($query_file_2_cnt > 0){?>		
		<?
			for($opt_i=0; $opt_i<sizeof($file_2_title_arr); $opt_i++){
				$file_2_idx = trim($file_2_idx_arr[$opt_i]);
		...
		?>
			...
			<?if($query_file_2_cnt > 0){?>	
				...
				?>
						...
						<?if($row_file_4['analysis_report_service_data_quiz_no'] == $file_2_quizno){?>
							<?
								$sql_file_1 = "select idx from wise_analysis_report_service_data_quiz where 1 and service_option_idx = '".$service_option_idx."' and analysis_report_service_data_quiz_no = '".$row_file_4['analysis_report_service_data_quiz_no']."' and service_data_idx is null and analysis_report_service_data_quiz_val >= '".$row_tot_avg."'";
								//echo $sql_file_1."<br>";
								$query_file_1 = mysqli_query($gconnet,$sql_file_1);
								$query_file_1_cnt = mysqli_num_rows($query_file_1);
							?>
