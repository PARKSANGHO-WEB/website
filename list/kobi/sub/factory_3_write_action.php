...
$attach_count_ext_1 = trim(sqlfilter($_REQUEST['attach_count_ext_1'])); 
...
$view_id = $_SESSION['member_coinc_id'];
	...
	for($file_i=0; $file_i<$attach_count_1; $file_i++){ // 설정된 갯수만큼 루프 시작
		...
		if ($_FILES['file0_'.$file_i]['size']>0){ // 파일이 있다면 업로드한다 시작
			...
			if($bbs_code == "data_list"){
				...
				$i_width2 = "";
				$i_height2 = "";
				$file_c = uploadFileThumb_1($_FILES, "file0_".$file_i, $_FILES['file0_'.$file_i], $_P_DIR_FILE,$i_width,$i_height,$i_width2,$i_height2,$watermark_sect);
			} else {
...
if($result){
	...
	<!--
		alert('상담실(Q&A) 등록이 정상적으로 완료 되었습니다.');
	...
	<SCRIPT LANGUAGE="JavaScript">
	<!--
	alert('상담실(Q&A) 의뢰 등록중 오류가 발생했습니다.');
	//-->
	</SCRIPT>
	<?}?>