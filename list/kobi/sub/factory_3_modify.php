...
<? include $_SERVER["DOCUMENT_ROOT"]."/pro_inc/include_default.php"; // 공통함수 인클루드 ?>
    ...
    <script>
		...
		var all_s = document.getElementById('all_s').checked;
		var all_b = document.getElementById('all_b').checked;
		var all_n = document.getElementById('all_n').checked;
		var all_t = document.getElementById('all_t').checked;
		var all_any = document.getElementById('all_any').checked;
		...
		if(!all_s &&!all_b&&!all_n&&!all_t&&!all_any){
				alert('분야를 체크 해주세요.');
				return false;	
		}
...
<SCRIPT LANGUAGE="JavaScript">
	alert('상담실(Q&A)게시판은 회원만 수정이 가능합니다.');
...
<?
	$sel_sql = "select * from member_info where user_id='".$_SESSION['member_coinc_id']."'";
...
?>
<SCRIPT LANGUAGE="JavaScript">
	alert('해당글은 작성한 본인만 수정이 가능합니다.');
	location.href =  location.href =  history.back();
</SCRIPT>
<?
exit;
}
?>
        ...
        <div class="fac3w-1">
            <p class="qna-ment">컨설팅에 관한 <span class="bold">궁금증</span>을 남겨주세요.<br />코비인사이트의<span class="bold"> 전문가들</span>이 친절하게 답변 드립니다.</p>
            <div id="ourHolder">
                   <form action="factory_3_modify_action.php" name="frm" id="frm" method="post" enctype="multipart/form-data">
				     <input type="hidden" name="bbs_code" value="qna">
				   	<input type="hidden" name="ip" value="<?= $_SERVER['REMOTE_ADDR']?>">
					 <input type="hidden" name="writer" readonly value="<?=$sel_row2[writer]?>">
                            <input type="hidden" name="email" readonly value="<?=$sel_row2[user_id]?>">
                            <input type="hidden" name="cell" readonly value="<?=$sel_row2[cell]?>">
							<input type="hidden" name="com_name" readonly value="<?=$sel_row2[com_name]?>"> 
							<input type="hidden" name="idx" value="<?=$idx?>"/>
					<div class="fc3w-form">
						<div class="fc3w-title">
                            <p>제목</p>
                            <input type="text" name="subject" value="<?=$sel_row2[subject]?>">
                        </div>
						
						<div class="fc3w-cate">
                            <p>분야</p>
                            <label for="all_s">
                                <input name="bbs_sect2" id="all_s" type="radio" value="세무" <?if($sel_row2[bbs_sect2] == "세무") echo 'checked="checked"';?>>
                                세무
                            </label>
                            <label for="all_b">
                                <input name="bbs_sect2" id="all_b"  type="radio" value="법무" <?if($sel_row2[bbs_sect2] == "법무") echo 'checked="checked"';?>>
                                법무
                            </label>
                            <label for="all_n">
                                <input name="bbs_sect2" id="all_n"  type="radio" value="노무" <?if($sel_row2[bbs_sect2] == "노무") echo 'checked="checked"';?>>
                                노무
                            </label>
                            <label for="all_t">
                                <input name="bbs_sect2" id="all_t"  type="radio" value="인증" <?if($sel_row2[bbs_sect2] == "인증") echo 'checked="checked"';?>>
                                인증
                            </label>
                            <label for="all_any">
                                <input name="bbs_sect2" id="all_any"  type="radio" value="기타" <?if($sel_row2[bbs_sect2] == "기타") echo 'checked="checked"';?>>
                                기타
                            </label>
                        </div>
                        <div class="fc3w-cont">
                            <p>내용</p>
                            <textarea name="content" id="content" cols="30" rows="10"><?=$sel_row2[content]?></textarea>
                        </div>
	<!--
                        <div class="fc3w-keyword">
                            <p>키워드</p>
                            <input name='basic' value='' placeholder="키워드를 입력하세요." autofocus>
                        </div>
-->
                      <!--  <script>
						...
						<div class="fc3w-file">
                            <p>파일첨부</p>
							<div class="file-input" id="docu_area_no_parent">
									<span class="button"  onclick="javascript:addForm_1();set_upload_file();">파일선택</span>
								<span class="label" data-js-label="">선택된 파일 없음</span>
							</div>
                        </div>  
                        
                        <div class="fc3w-file uploaded">
                            <div class="file-input uploaded-input" id="docu_area_no_parent1">

					<?
						$sql_file = "select idx,file_org,file_chg from board_file where 1=1 and board_tbname='board_content' and board_code = '".$sel_row2['bbs_code']."' and board_idx='".$sel_row2['idx']."' order by idx asc ";
						
						$query_file = mysqli_query($gconnet,$sql_file);
						//$row_file = mysqli_fetch_array($query_file);
					?>
						<input type="hidden" name="attach_count_1" id="attach_count_1" value="<?=mysqli_num_rows($query_file)?>"/>
						<input type="hidden" name="attach_count_ext_1" id="attach_count_ext_1" value="<?=mysqli_num_rows($query_file)?>"/>	
                          <!-- <input type="file" required="no" message="기타 첨부자료" name="file_0" id="file_0">
					...
					<?
						for($i_file=0; $i_file<mysqli_num_rows($query_file); $i_file++){
							...
							$k_file = $i_file+1;
					?>	
						<input type="hidden" name="file_idx_0_<?=$i_file?>"  id="file_idx_0_<?=$i_file?>" value="<?=$row_file['idx']?>" />
						<input type="hidden" name="file_old_name_0_<?=$i_file?>" id="file_old_name_0_<?=$i_file?>"  value="<?=$row_file['file_chg']?>" />
						<input type="hidden" name="file_old_org_0_<?=$i_file?>" id="file_old_org_0_<?=$i_file?>"  value="<?=$row_file['file_org']?>" />

						<input type="file" style="display:none;" required="no" message="참고자료" name="file0_<?=$i_file?>" id="file0_<?=$i_file?>" onchange="view_upload_file('<?=$i_file?>');"><!-- 파일 업로드 후 UI -->
						<input type="checkbox" name="del_org_0_<?=$i_file?>" id="del_org_0_<?=$i_file?>" value="Y" style="display:none;">
						<div class="fc3w-file inline" id="docu_area_<?=$i_file?>" style="display:;">
						<div class="file-input inline" id="docu_area_name_<?=$i_file?>"><?=$row_file['file_org']?></div><a href="javascript:del_uploaded_photo('<?=$i_file?>');"><img src="../img/icn-x-delete.png" alt=""/></a></div><!-- 파일 업로드 후 UI -->
						<?}?>
                            </div>
                      ...
                      <!--   <script>
                        ...
                        <div class="fc3w-sub">
                            <button type="button" onclick="go_submit();">제출</button>
                        </div>
                    </div>
                </form>
            </div>
...
var count_1 = <?=mysqli_num_rows($query_file)?>;          

function set_upload_file(){
		if(Number($("#attach_count_1").val()) > 6){
		} else {
			//var num = Number($("#attach_count_1").val());
			/*num = num -1;
			if(num < 0){
				num = 0;
			}*/
			
			var count_pre = count_1-1;
			if (count_pre <= 0){
				count_pre = 0;
			}
			//alert(num);
			$("#file0_"+count_pre+"").click();
		}
	}

	function view_upload_file(num){
		$("#docu_area_"+num+"").show();
		
		var fileValue = $("#file0_"+num+"").val().split("\\");
		var fileName = fileValue[fileValue.length-1]; // 파일명

		if (fileName == ""){
			alert("no");
		} else {
			if($("#file_old_name_0_"+num+"").val() == ""){
				//$("#docu_area_no_"+num+"").html(fileName);
			}
			$("#docu_area_name_"+num+"").html(fileName);
			$("#btn_del_"+num+"").show();
		...
		{
			fileSize = $("#docu_"+num+"").files.item(0).filesize;
		}
	...
	function del_selected_photo(num){
		 $("#file0_"+num+"").val("");
		 $("#docu_area_name_"+num+"").html("");
		 if($("#file_old_name_0_"+num+"").val() == ""){
			 //$("#docu_area_no_"+num+"").html("선택된 파일 없음");
		 }
		 $("#docu_area_"+num+"").hide();
		 var fcnt = Number($("#attach_count_1").val());
		 fcnt = fcnt -1;
	...
	function del_uploaded_photo(num){
		 //$("#file0_"+num+"").val("");
		 $("#del_org_0_"+num+"").prop("checked", true);
		 $("#docu_area_name_"+num+"").html("");
		 $("#docu_area_"+num+"").hide();
		 var fcnt = Number($("#attach_count_1").val());
		 fcnt = fcnt -1;
	...
	function addForm_1(){
	if(Number($("#attach_count_1").val()) >6){
		alert("파일은 6개 까지 추가할 수 있습니다.");
		return false;
	} else {
		var addedFormDiv = document.getElementById("docu_area_no_parent1");
		
		var str = "";
	
		str+='<input type="file" style="display:none;" required="no" message="참고자료" name="file0_'+count_1+'" id="file0_'+count_1+'" onchange="view_upload_file(\''+count_1+'\');"><!-- 파일 업로드 후 UI --><div class="fc3w-file" id="docu_area_'+count_1+'"><div class="file-input" id="docu_area_name_'+count_1+'"></div><a href="javascript:del_selected_photo(\''+count_1+'\');"><img src="../img/icn-x-delete.png" alt="" id="btn_del_'+count_1+'" style="display:none;"/></a></div><!-- 파일 업로드 후 UI -->'; // 추가할 폼(에 들어갈 HTML)
