...
?>
        ...
        <div class="fac2_v">
           <form action="#">
			...
			<?
			$i=1;
			$sql_file = "select file_org,file_chg from board_file where 1=1 and board_tbname='board_content' and board_code = '".$row['bbs_code']."' and board_idx='".$row['idx']."' order by idx asc limit 1,5";
			$query_file = mysqli_query($gconnet,$sql_file);
			while($row_file = mysqli_fetch_array($query_file)){
			?>
                                    <img src="<?=$_P_DIR_WEB_FILE?><?=$bbs_code?>/img_thumb/<?=$row_file['file_chg']?>" alt="">
             <?
					$i++;
				}
			 ?>                   
		...
		<? } else { ?>	
				<a class="prev-write" href="factory_2_view.php?idx=<?=$up_row['idx']?>&bbs_code=<?=$bbs_code?>&bbs_sect2=<?=$bbs_sect2?>">다음글</a>	
		<?}?>	
		<?
		...
		?>
				<a class="next-write" href="factory_2_view.php?idx=<?=$down_row['idx']?>&bbs_code=<?=$bbs_code?>&bbs_sect2=<?=$bbs_sect2?>">이전글</a>           
		<?}?>
                </div>
            </form>
        </div>
