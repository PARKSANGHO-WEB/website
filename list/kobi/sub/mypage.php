...
function go_submit() {
			...
			var consult = document.getElementById('consult').checked;
			var company = document.getElementById('company').checked;


			if(document.frm.cell1.value == ''){
				alert('휴대폰 첫자리를 선택하세요');
				document.frm.cell1.focus();
				return false;
			}

			if(document.frm.cell2.value == ''){
				alert('휴대폰 두번째자리를 입력하세요');
				document.frm.cell2.focus();
				return false;
			}

			if(document.frm.cell3.value == ''){
				alert('휴대폰 마지막자리를 입력하세요');
				document.frm.cell3.focus();
				return false;
			}
			if(document.frm.com_name.value == ''){
				alert('소속회사를 입력하세요');
				document.frm.com_name.focus();
				return false;
			}
			if(document.frm.jobposition.value == ''){
				alert('직책을 입력하세요');
				document.frm.jobposition.focus();
				return false;
			}
			if(document.frm.com_addr.value == ''){
				alert('회사주소를 입력하세요');
				document.frm.com_addr.focus();
				return false;
			}
        ...
        <section>
            <div class="join">
                <p>마이페이지</p>
                <span>입력하신 정보를 확인하실 수 있습니다.</span>
            </div>
<?
	...
	$cell = explode('-',$sel_row[cell]);
	$birthday = explode('-',$sel_row[birthday]);
?>
            <div class="join_f">
                <form name="frm" id="frm" action="mypage_action.php"  method="post"  enctype="multipart/form-data">
                     <div class="form-wrap">
					<?if($sel_row[sns_kind] =="kt" || $sel_row[sns_kind] =="gp"){?>
					  <div class="line_2">
                            <span class="title">
                                이름
                            </span>
                            <input type="text" placeholder="홍길동" disabled value="<?=$sel_row[user_name]?>">
                        </div>
					<?}?>
<!--
                        ...
                        <div class="line_3">
                            ...
                            <input type="text" placeholder="gildong1@naver.com" disabled value="<?=$sel_row[user_id]?>">
                        </div>
					<?
						if($sel_row[sns_kind] !="kt" && $sel_row[sns_kind] !="nv" && $sel_row[sns_kind] !="gp"){
					?>
                        <div class="line_4">
                            <span class="title">
                                비밀번호
                            </span>
                            <input id="password" type="password" placeholder="비밀번호를 입력하세요." name="user_pwd" value="">
                            <input id="confirm_password" type="password" placeholder="비밀번호를 다시 한 번 입력해주세요." name="user_pwd_re" value="">
						...
						<div class="line_5">
                            ...
                            <input   maxlength="6" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" placeholder="123456" disabled value="<?=$birthday[0]?>">
                            <span class="hypen" placeholder="생년월일">-</span>
                            <input  maxlength="1"oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"  placeholder="7" disabled value="<?=$birthday[1]?>">
                            <span>주민등록번호 뒷 자리는 첫 한 자리만 입력해주세요.</span>
                        </div>
-->
                        <div class="line_6">
                            <span class="title">
                            ...
                            <select name="cell1"  id="pnumber">
                                <option value="not">선택</option>
                                <option value="010" <?if($cell[0]=='010') echo 'selected="selected"';?>>010</option>
                                <option value="011" <?if($cell[0]=='011') echo 'selected="selected"';?>>011</option>
                                <option value="017" <?if($cell[0]=='017') echo 'selected="selected"';?>>017</option>
                                <option value="070" <?if($cell[0]=='070') echo 'selected="selected"';?>>070</option>
                                <option value="080" <?if($cell[0]=='080') echo 'selected="selected"';?>>080</option>
                            </select>
                            <span class="hypen">-</span>
                            <input type="text" placeholder="0000"   name="cell2" maxlength="4" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" value="<?=$cell[1]?>"/>
                            <span class="hypen">-</span>
                            <input type="text"maxlength="4"  placeholder="0000"  name="cell3" ninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" value="<?=$cell[2]?>" />
                        </div>
                        ...
                        <div class="line_8">
                            ...
                            <input type="text" placeholder="회사명" name="com_name" value="<?=$sel_row[com_name]?>">
                        </div>
                        <div class="line_9">
                            <span class="title">
                                직책
                            </span>
                            <input type="text" placeholder="직책" name="jobposition" value="<?=$sel_row[jobposition]?>">
                        </div>
                        <div class="line_10">
                            <span class="title">
                                회사주소
                            </span>
                            <input type="text" placeholder="서울시 노원구 공릉동"  name="com_addr" value="<?=$sel_row[com_addr]?>">
                        </div>
                        <div class="line_11">
                            <span class="title">
                                구분
                            </span>
                            <label class="consult" for="consult"><input type="radio" name="consult" id="consult" <?if($sel_row[consult]=="컨설턴트") echo 'checked="checked"';?> value="컨설턴트">컨설턴트</label>
                            <label class="company" for="company"><input type="radio" name="consult" id="company" <?if($sel_row[consult]=="기업임직원") echo 'checked="checked"';?> value="기업임직원">기업임직원</label>
                        </div>
                        <div class="form_btn">
                            <button type="button" onclick="javascript:history.back();">취소</button>
                            <button type="button" onclick="go_submit();">수정완료</button>
                        </div>
                    </div>
                </form>
            </div>
