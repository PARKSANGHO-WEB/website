...
<? include $_SERVER["DOCUMENT_ROOT"]."/pro_inc/include_default.php"; // 공통함수 인클루드 ?>
        ...
        <div class="header-t">
            ...
            <div class="nav">
                ...
                <div class="menu_b">
                    ...
                    <ul>
                        <li class="off-item">
                            <a class="big_m"  href="./about.php#about_1">About Us</a>
                            <div class="menu_in">
                                <ul>
                                    <li>
                        ...
                        <li class="off-item">
                            <a class="big_m on" href="./factory_1.php">Business Factory</a>
                            <div class="menu_in">
                                <ul>
                                    <li>
    ...
    <div id="navbar" class="about_tab f3-tab in">
        <div class="at-wrap">
        <a href="../main.php" class="home">
            <img class="" src="../img/sub/home.png" alt="">
        </a>
            <a href="./factory_1.php" class="dot">진행현황</a>
            <a href="./factory_2.php" class="dot">컨설팅 사례</a>
            <a href="./factory_3.php" class="dot active">상담실(Q&A)</a>
            <a href="./factory_4.php" class="dot">FACTORY(자료실)</a>
            <a href="./factory_5.php" class="dot">공작소(자료제작)</a>
        </div>
    </div>
...
$total_param = 'field='.$field.'&keyword='.$keyword.'&bbs_code='.$bbs_code;
//$total_param2 = 'field='.$field.'&keyword='.$keyword.'&bbs_code='.$bbs_code.'&bbs_sect2='.$bbs_sect2."&go_div=board_list_area";
$total_param2 = 'field='.$field.'&keyword='.$keyword.'&bbs_code='.$bbs_code.'&bbs_sect2='.$bbs_sect2;
...
if($bbs_sect2){
	$where2 .= " and a.bbs_sect2 = '".$bbs_sect2."' ";
}

$query_cnt2 = "select is_popup from board_content a where 1=1 and is_popup='Y'".$where.$where2;
$result_cnt2 = mysqli_query($gconnet,$query_cnt2);
$num2 = mysqli_num_rows($result_cnt2);
//echo $num2;

if($num2 >0 && $pageNo==1){
$pageScale = 8+2-$num2; // 페이지당 20 개씩
}else{
...
$query = "select * from board_content a where 1=1 ".$where.$where2.$order_by." limit ".$StarRowNum." , ".$EndRowNum;
$query_cnt = "select idx from board_content a where 1=1 ".$where.$where2;
...
?>
    <script>
		...
		location.href = "./factory_3_view.php?idx="+no+"&bbs_code="+bcode+"&pageNo=<?=$pageNo?>&field=<?=$field?>&keyword=<?=$keyword?>&bbs_sect2=<?=$bbs_sect2?>";
	}
	
	function go_list(){
		//location.href = "factory_3.php?<?=$total_param?>&go_div=board_list_area";
		location.href = "factory_3.php?<?=$total_param?>";
	}
	
	function go_list2(bbs_sect2){
		//location.href = "factory_3.php?<?=$total_param?>&bbs_sect2="+bbs_sect2+"&go_div=board_list_area";
		location.href = "factory_3.php?<?=$total_param?>&bbs_sect2="+bbs_sect2;
	}
    ...
    function fnMove(divname){
      ...
      var go_top = Number(offset.top)+10; // + 뒤의 수치는 적절하게 조절 
        $('.fac_ban').removeClass('in');
        $('.f3-tab').removeClass('in');
        $('.con-ment').removeClass('in');
        $('html, body').animate({scrollTop : go_top}, 0);
    }

   $(document).ready(function(){
      fnMove("<?=$_REQUEST['go_div']?>");
   });
</script>
        <div class="fac3-1 " id="board_list_area">
            <p class="con-ment in">컨설팅에 관한 <span class="bold">궁금한 점</span>을 남겨주세요.<br />코비인사이트의 <span class="bold">전문가들</span>이 친절하게 답변 드립니다.</p>
             <div class="filter-btn">
               <ul id="filterOptions">
                    <li class="<?if($bbs_sect2=='')echo 'active';?>"><a class="all" href="javascript:go_list();">전체</a></li>
                    <li class="<?if($bbs_sect2=='세무')echo 'active';?>"><a class="semu" href="javascript:go_list2('세무');" >세무 컨설팅</a></li>
                    <li class="<?if($bbs_sect2=='법무')echo 'active';?>"><a class="bub" href="javascript:go_list2('법무');">법무 컨설팅</a></li>
                    <li class="<?if($bbs_sect2=='노무')echo 'active';?>"><a class="nomu" href="javascript:go_list2('노무');">노무 컨설팅</a></li>
                    <li class="<?if($bbs_sect2=='인증')echo 'active';?>"><a class="serty" href="javascript:go_list2('인증');">인증 컨설팅</a></li>
                    <li class="<?if($bbs_sect2=='기타')echo 'active';?>"><a class="any" href="javascript:go_list2('기타');">기타</a>
                 </ul>
            </div>
            <div class="fc1-search">
			<div class="write-btn">
                    <button onclick='location.href="./factory_3_write.php"' type="button">문의하기</button>
                </div>
                <div class="sr-wrap">
                    <form name="s_mem" id="s_mem" method="post" action="factory_3.php">
                    <input type="hidden" name="bbs_code" value="<?=$bbs_code?>"/>
					<input type="hidden" name="bbs_sect2" value="<?=$bbs_sect2?>"/>
                    <select name="field" id="sear-cate">
                        <option value="not" <?=$field=="not"?"selected":""?>>전체</option>
                    ...
                    <button type="button" onclick="s_mem.submit();"></button>
					</form>
                </div>
            </div>
            <div id="ourHolder">
                       <form action="#">
                        <div class="fc3-table">
                            <table>
                                <tr class="semu bub nomu any serty">
                                    <th>번호</th>
                                    <th>분야</th>
                                    <th>제목</th>
                                    <th>답변글</th>
					...
					<?
			if($pageNo == 1){
			$query_sub_notice="select idx,bbs_code,p_no,ref,step,depth,subject,writer,cnt,reco,write_time,is_del,del_sect,bbs_sect,bbs_sect2 from board_content a where 1=1 and is_popup='Y' and step='0' and bbs_code = '".$bbs_code."' ".$order_by;
			...
			if(mysqli_num_rows($result_sub_notice)==0) {
				...
				for ($i_sub_notice=0; $i_sub_notice<mysqli_num_rows($result_sub_notice); $i_sub_notice++){ // 공지사항 지정글 루프
					...
					$bbs_sect2_str="";
				if($row_sub_notice[bbs_sect2]=="세무"){
					$bbs_sect2_str="semu";
				}else if($row_sub_notice[bbs_sect2]=="법무"){
					$bbs_sect2_str="bub";
				}else if($row_sub_notice[bbs_sect2]=="노무"){
					$bbs_sect2_str="nomu";
				}else if($row_sub_notice[bbs_sect2]=="인증"){
					$bbs_sect2_str="serty";
				}else if($row_sub_notice[bbs_sect2]=="기타"){
					$bbs_sect2_str="any";
				}
				$com_sql_cnt = "select count(*) as cnt from board_comment where board_tbname='board_content' and board_code='qna' and board_idx='".$row_sub_notice[idx]."'";
				$com_result_cnt = mysqli_query($gconnet,$com_sql_cnt);
				$com_row_cnt = mysqli_fetch_array($com_result_cnt);

				
				$writer_length = mb_strlen ($row_sub_notice[writer],"utf-8");
				if($writer_length==3){
				$left = mb_substr ($row_sub_notice[writer], 0, 1,"utf-8");
				$right = mb_substr ($row_sub_notice[writer], 2, 1,"utf-8");
				$writer_str=$left."O".$right;
				}else{
				$writer_str=$row_sub_notice[writer];
				
				}

					
		?>
							<tr class="item noticeitem <?=$bbs_sect2_str?>">
                                    <td><span class="dice">[공지]</span></td>
                                    <td><span class="type"> <?=$row_sub_notice[bbs_sect2]?> </span></td>
                                    <td onclick="javascript:go_view('<?=$row_sub_notice[idx]?>','<?=$row_sub_notice['bbs_code']?>');"><span class="title"><?=$row_sub_notice[subject]?></span></td>
                                    <td><span class="ans"><?=$com_row_cnt[cnt]?></span></td>
								    <td><?=$writer_str?></td>
                                    <td><span class="date"> <?=substr($row_sub_notice[write_time],0,10)?></span></td>
                                    <td><span class="view"><?=$row_sub_notice[cnt]?></span></td>
                              </tr>		
	...
	?>	
			...
			for ($i=0; $i<mysqli_num_rows($result); $i++){
				...
				if($row[bbs_sect2]=="세무"){
					$bbs_sect2_str="semu";
				}else if($row[bbs_sect2]=="법무"){
					$bbs_sect2_str="bub";
				}else if($row[bbs_sect2]=="노무"){
					$bbs_sect2_str="nomu";
				}else if($row[bbs_sect2]=="인증"){
					$bbs_sect2_str="serty";
				}else if($row[bbs_sect2]=="기타"){
					$bbs_sect2_str="any";
				}
				$com_sql_cnt = "select count(*) as cnt from board_comment where board_tbname='board_content' and board_code='qna' and board_idx='".$row[idx]."'";
				$com_result_cnt = mysqli_query($gconnet,$com_sql_cnt);
				$com_row_cnt = mysqli_fetch_array($com_result_cnt);

				
				$writer_length = mb_strlen ($row[writer],"utf-8");
				if($writer_length==3){
				$left = mb_substr ($row[writer], 0, 1,"utf-8");
				$right = mb_substr ($row[writer], 2, 1,"utf-8");
				$writer_str=$left."O".$right;
				}else{
				$writer_str=$row[writer];
				
				}

			?>
								<tr class="item <?=$bbs_sect2_str?>">
                                    <td><span class="dice"><?=$listnum?></span></td>
                                    <td><span class="type"> <?=$row[bbs_sect2]?> </span></td>
                                    <td onclick="javascript:go_view('<?=$row[idx]?>','<?=$row['bbs_code']?>');"><span class="title"><?=$row[subject]?></span></td>
                                    <td><span class="ans"><?=$com_row_cnt[cnt]?></span></td>
								    <td><?=$writer_str?></td>
                                    <td><span class="date"> <?=substr($row[write_time],0,10)?></span></td>
                                    <td><span class="view"><?=$row[cnt]?></span></td>
                                </tr>
				<?
				}
			?>
                               <!--  <tr class="item bub">
                                    <td><span class="dice">566</span></td>
                                    <td><span class="type">법무</span></td>
                                    <td onclick='location.href="./factory_3_view.php"'><span class="title">제목입력창입니다.제목입력창입니다.제목입력창입니다.제목입력창입니다.제목입력창입니다.제목입력창입니다.제목입력창입니다.제목입력창입니다.</span></td>
                                    <td><span class="ans">1</span></td>
                                    <td><span class="date">2020-07-21</span></td>
                                    <td><span class="view">80</span></td>
                                </tr>
                                <tr class="item semu">
                                    <td><span class="dice">566</span></td>
                                    <td><span class="type">세무</span></td>
                                    <td onclick='location.href="./factory_3_view.html"'><span class="title">제목입력창입니다.제목입력창입니다.제목입력창입니다.제목입력창입니다.제목입력창입니다.제목입력창입니다.제목입력창입니다.제목입력창입니다.</span></td>
                                    <td><span class="ans">1</span></td>
                                    <td><span class="date">2020-07-21</span></td>
                                    <td><span class="view">80</span></td>
                                </tr>
                            ...
                            <div class="page_btn">
                               <?include $_SERVER["DOCUMENT_ROOT"]."/pro_inc/paging3.php";?>
                            </div>
                        </div>
                    </form>
            </div>
             ...
             <!-- <script>
                $(document).ready(function() {
                  $('#filterOptions li a').click(function() {
                    ...
                    if(ourClass == 'all') {
                      // show all our items
                      $('#ourHolder').find('tr.item').show();
                    }
                    else {
                      // hide all elements that don't share ourClass
                      $('#ourHolder').find('tr:not(.' + ourClass + ')').hide();
                      // show all elements that do share ourClass
                      $('#ourHolder').find('tr.' + ourClass).show();
                    }
                    return false;
                  });
                });
