...
$total_param =  'pageNo='.$pageNo.'&field='.$field.'&keyword='.$keyword.'&bbs_code='.$bbs_code.'&bbs_sect2='.$bbs_sect2;
if($bbs_sect2){
...
?>
<SCRIPT LANGUAGE="JavaScript">
	alert('상담실(Q&A)게시판은 회원만 열람이 가능합니다.');
...
$sel_sql = "SELECT * FROM board_content a where 1=1 and a.idx = '".$idx."' and a.bbs_code='".$bbs_code."'".$where."and user_id='".$_SESSION['member_coinc_id']."'";
$sel_query = mysqli_query($gconnet,$sel_sql);
...
function go_modify(no){
		location.href = "factory_3_modify.php?idx="+no+"&<?=$total_param?>";
}
function go_delete(no){
		location.href = "factory_3_delete_action.php?idx="+no+"&<?=$total_param?>";
}
</script>
        <div class="fac3v-1">
            <div id="ourHolder">
                       <form action="#">
                        <div class="fc3v-table">
							...
							<table>
                                ...
                                <tr class="v-info">
                                    <td>
                                        <div class="fcw-p">
                                             <span class="fcw-t">작성자</span>   
                                             <span class="fcw-c"><?=$writer_str?></span>   
                                        </div>
                                        <div class="fcw-d">
                                             <span class="fcw-dt">작성일</span>   
                                             <span class="fcw-date"><?=$wdate_str?></span>   
                                        </div>
                                    </td>
                                </tr>
                                <tr class="v-con">
                                ...
                                <tr class="file-down">
									...
									?>
									   ...
									   <?
											...
											<?}?>
											<a href="/pro_inc/download_file.php?nm=<?=$row_file['file_chg']?>&on=<?=$row_file['file_org']?>&dir=<?=$row['bbs_code']?>" target="_blank" download ><?=nl2br($row_file['file_org'])?></a></div>
											<?
												$i++;
											}
					...
					<?
						$cnt_sql = "select count(*) as cnt from board_comment where board_tbname='board_content' and board_code='qna' and board_idx='".$row[idx]."'";
						$cnt_result = mysqli_query($gconnet,$cnt_sql);
						$cnt_row = mysqli_fetch_array($cnt_result);
						
						if($cnt_row[cnt]>0){
					?>
						<div class="fc3v-atable">
                            <table>
                                <tr class="v-title">
                                    <td>
                                        <span class="write-t">[ <?=$row[subject]?>]</span>에 대한 답변입니다.
                                    </td>
                                </tr>
								<?
									$com_sql = "select * from board_comment where board_tbname='board_content' and board_code='qna' and board_idx='".$row[idx]."'";
									$com_result = mysqli_query($gconnet,$com_sql);
									while($com_row = mysqli_fetch_array($com_result)){
									$wdate=date_create($com_row[write_time]);
									$wdate_str=date_format($wdate,'Y.m.d');	


									$writer_length = mb_strlen ($com_row[writer],"utf-8");
				if($writer_length==3){
				$left = mb_substr ($com_row[writer], 0, 1,"utf-8");
				$right = mb_substr ($com_row[writer], 2, 1,"utf-8");
				$writer_str=$left."O".$right;
				}else{
				$writer_str=$com_row[writer];
                                ...
                                <tr class="v-con">
                                    <td>
                                        <p><?=nl2br($com_row[content])?></p>
										<?if($com_row[file_chg]){?>
											<img src="<?=$_P_DIR_WEB_FILE.$row['bbs_code']?>/img_thumb/<?=$com_row[file_chg]?>" border="0">	
										 <?}?>
                                    </td>
                                </tr>
								<?}?>
						...
						?>
<?
if($_SESSION['member_coinc_id']==$row[user_id]){
?>
						<div class="back-btn mody">
                            <button onclick="go_modify(<?=$idx?>)" type="button">수정하기</button>
                        </div>
						<div class="back-btn dele">
                            <button onclick="go_delete(<?=$idx?>)" type="button">삭제하기</button>
                        </div>
<?
}
?>
                        <div class="back-btn">
                            ...
                            <button onclick="javascript:goBack();" type="button">목록으로</button>
                            
                            
                        <?
                                if($bbs_sect2){

                                $up_query = "select idx,subject,bbs_sect2 from board_content where bbs_code='".$bbs_code."' and write_time > '".$row[write_time]."' and step='0' and bbs_sect2='".$bbs_sect2."' order by write_time asc limit 0,1";

                                }else if(!$bbs_sect2){

                                $up_query = "select idx,subject,bbs_sect2 from board_content where bbs_code='".$bbs_code."' and write_time > '".$row[write_time]."' and step='0' order by write_time asc limit 0,1";
                                }
                            ...
                            <? } else { ?>	
                                    <a class="prev-write" class="prev-write" href="factory_3_view.php?idx=<?=$up_row['idx']?>&bbs_code=<?=$bbs_code?>&bbs_sect2=<?=$bbs_sect2?>">다음글</a>	
                            <?}?>
                            <?


                            if($bbs_sect2){
                                $down_query = "select idx,subject,bbs_sect2 from board_content where bbs_code='".$bbs_code."' and write_time < '".$row[write_time]."' and step='0' and bbs_sect2='".$bbs_sect2."' order by write_time desc limit 0,1";
                            }else if(!$bbs_sect2){

                                $down_query = "select idx,subject,bbs_sect2 from board_content where bbs_code='".$bbs_code."' and write_time < '".$row[write_time]."' and step='0' order by write_time desc limit 0,1";
                            }
                                if($my_list){ // 1:1 게시판일때
                                    if($view_idx){
                            ...
                            ?>
                                    <a class="next-write" href="factory_3_view.php?idx=<?=$down_row['idx']?>&bbs_code=<?=$bbs_code?>&bbs_sect2=<?=$bbs_sect2?>">이전글</a>           
                            <?}?>	
                        </div>
                    </form>
                    <script>
