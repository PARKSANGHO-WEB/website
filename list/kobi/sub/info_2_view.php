...
$total_param =  'pageNo='.$pageNo.'&field='.$field.'&keyword='.$keyword.'&bbs_code='.$bbs_code;
if($bbs_code == "reviews"){
...
<SCRIPT LANGUAGE="JavaScript">
	...
	location.href =  "info_2.php?<?=$total_param?>";
	//-->
</SCRIPT>
<?
exit;
...
?>
        <div class="notice_v">
           <form action="#">
				...
				<?
			$up_query = "select idx,subject from board_content where bbs_code='".$bbs_code."' and write_time > '".$row[write_time]."' and step='0' order by write_time asc limit 0,1";
		...
		<? } else { ?>	
				<a class="prev-write" href="info_2_view.php?idx=<?=$up_row['idx']?>&bbs_code=<?=$bbs_code?>">다음글</a>	
		<?}?>	         
		<?
		...
		?>
				<a class="next-write" href="info_2_view.php?idx=<?=$down_row['idx']?>&bbs_code=<?=$bbs_code?>">이전글</a>           
		<?}?> 
